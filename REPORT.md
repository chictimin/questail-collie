...
| 2 (도구 재설계, 측정 전) | 도구 4개→9개 재설계 + 정답지 `expectedTools` 교체 + 오프라인 드라이런 하네스(`src/dryrun.ts`) | 1·2차 오답 분석에서 "관련 없는 근거를 훑는 것"이 도구 입도 문제로 확인됐기 때문. 업적률 질문에 `lookup_library`로 라이브러리 행 전체를 긁는 식이다. 명세는 도구를 "어느 근거를 조회해야 하는가"의 단위로 정의하고 개수 제약이 없다 | 도구 선택이 근거 단위로 나뉘고, 라우터 성능을 분류기·답변과 분리해 잰다(`npx tsx src/dryrun.ts` → tool N/27) | (측정 후 기입) | (측정 후 기입) |
