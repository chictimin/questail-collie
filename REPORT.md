...
게임 라이브러리 라우팅 에이전트. 흩어진 게임 이력을 md로 아카이빙하는 QuestTail의 저장소를 질의 대상으로 삼았다. 게임은 플레이타임·업적률·장르·별점·기피 사유처럼 출처와 성격이 다른 데이터 축이 한곳에 모여 있어, "근거의 출처 계층으로 카테고리를 가르는" 라우팅 설계의 재료가 된다. 일반 상식(LLM 내부 지식)으로 답하기 쉽고 근거 확인이 어려운 질문(내 별점·내 플레이타임)이 자연히 나와, 근거 이탈 검증을 걸기 좋다.
...
| 2 (도구 재설계) | 도구 4개→9개 재설계 + 정답지 `expectedTools` 교체 + 오프라인 드라이런 하네스(`src/dryrun.ts`) | 1·2차 오답 분석에서 "관련 없는 근거를 훑는 것"이 도구 입도 문제로 확인됐기 때문. 업적률 질문에 `lookup_library`로 라이브러리 행 전체를 긁는 식이다. 명세는 도구를 "어느 근거를 조회해야 하는가"의 단위로 정의하고 개수 제약이 없다 | 도구 선택이 근거 단위로 나뉘고, 라우터 성능을 분류기·답변과 분리해 잰다(`npx tsx src/dryrun.ts` → tool N/27) | tool 23/27 (LLM 0회 드라이런, 직접 실행 확인. 구 기준선 11/26 대비 +12 — 단 분모 상이, 구 기준선은 3회 공통 26문항) | 유지 (근거 단위 선택이 라우터 분리 측정에서 동작. 분류 어긋남 4건 남음 — 사이클 3) |
| 3 (분류 정의 정합) | CLASSIFY_SYSTEM의 카테고리 정의를 `docs/_mapping.md` 기준으로 맞춤. 바뀐 정의 5줄은 아래 "사이클 3"절 | 사이클 2 드라이런의 틀린 4건이 전부 캐시 분류≠정답 분류였고, 어긋난 축 4개가 구 프롬프트 정의와 매핑표 정의의 차이와 대응했기 때문 | 분류 오차 해소 | tool 26/27 (+3, 직접 실행 확인). 분류 정확도 29/30. 남은 1건(C2-05)은 단일 잡음으로 판단해 추가 수정 없음 | 정합 완료 (본측정 대기) |
...
1. 도구 입도가 거칠어서 "관련 없는 근거를 훑는 것"이 구조적으로 강제되고 있었다. 명세가 도구 지표로 잡으라는 항목 중 하나가 과잉 호출이며, (3)절이 단일 도구 문항을 절반 이상 남긴 것도 과잉 호출을 오답으로 잡기 위해서다.
2. 업적률 하나 물었는데 `lookup_library`로 라이브러리 전체 행을 긁는 식이었다. C1-02·C1-06의 2차 답변 원문이 "플레이타임, 장르, 마지막 플레이 일정만 포함"으로 이를 보인다 — 물은 것과 온 것이 다르다.
3. 명세는 도구를 "어느 근거를 조회해야 하는가"의 단위로 정의하고 개수 제약이 없다. 근거 단위로 나누는 것이 명세에 맞는 방향이다.
4. 도구를 근거 단위로 다시 나누면 정답지도 따라 바뀔 수밖에 없고, 그 시점이 비교의 경계다. (7)절 회고의 "평가셋을 측정 사이에 고쳐 비교를 오염시켰다"와 같은 이유다 — 다른 점은 이번에는 고친 시점을 경계로 명시한다는 것이다.

이전 측정은 폐기가 아니라 구 도구 구성에서의 정본으로 남긴다. 1·2차 집계((4)절 표, tool 12/27→13/27)와 3회 반복(`20260917T070706`·`071057`·`072413` — 8개 결과 JSON 중 최신 3건. 공통 26문항에서 tool 변동 0건, 최신 회차 tool 11/26. JSON 행 대조로 확인. 세 회차 모두 구 정답지 기준이다: `072413`의 C1-02 기대가 `['lookup_library','search_docs']`, C1-04가 `['lookup_library','get_game_note','search_docs']`로 남아 있다)가 그 정본이다. 다음 회차부터는 새 정답지 기준으로 잰다.

### 도구 4개→9개 근거

도구 시그니처의 정본은 도구 재설계 계약이다. 아래는 그 계약을 옮긴 것이 아니라, 실패 문항 기준으로 평가자가 읽을 문장으로 다시 쓴 것이다. `lookup_library`·`get_game_note`·`get_taste_profile`·`search_docs`·`escalate` 5개는 그대로 두고, 모자란 자리 5개만 새로 만들었다. 카테고리 5개는 그대로다(명세 상한).

| 신규 도구 | 담당하게 된 질문 | 왜 기존 도구로는 안 됐는지 |
|---|---|---|
| `get_achievement_stats` | C1-02·C1-06 업적률 질문 | `lookup_library`는 플레이타임·장르·마지막 플레이가 담긴 행 전체를 긁고 업적 필드가 없어 "정보가 없다"에서 멈췄다(2차 답변 원문). 업적 공백의 이유(403 규정)는 D-D#업적-제약에 있어, 업적만 따로 조회하는 도구가 필요했다 |
| `get_wishlist` | C1-04 찜 membership, C1-07 위시 개수 | C1-04는 보유 인덱스만 뒤져 "없다"로 답했고, 찜 목록은 조회 대상이 아니었다(위 "실패 문항의 답변 원문" C1-04). C1-07은 개수 하나를 물었는데 취향 프로필 요약 전체(상위 장르·분포·기피·갭)를 긁었다(`tools.ts`의 `runTasteProfile` 출력 항목으로 확인) |
| `find_rating_playtime_gaps` | C2-04·C2-05 교차 질문 | 기존 정답은 `get_taste_profile`+`get_game_note` 3도구였으나, 갭 집계(D-C)가 근거에 없어 노트 조회가 빗나갔다(`note:unknown`). 플레이타임×별점 교차 상위를 직접 내는 도구가 필요했다 |
| `get_field_coverage` | C4-01·C4-04·C4-06 결측 질문 | "비어 있나"를 묻는데 행 전체를 긁었고, 장르 공백 탐색은 `emptyGenre` 인자 미전달로 실패했다(위 D2·C4-01 판정). 필드를 인자로 받는 결측 전용 도구가 필요했다 |
| `describe_schema` | C4-05 "어디서 보나" | 값 조회 3도구가 별점 위치 오해를 불렀다(금칙 위반 "library.md에 별점 열"). 파일·계층 위치를 답하는 도구가 필요했다 |

### 사이클 2·3 드라이런 결과

아래 숫자는 전부 LLM 0회 드라이런(`npx tsx src/dryrun.ts`, 직접 실행 확인)이며 본측정이 아니다. 답변 지표는 아직 없다.

| 회차 | tool | 비고 |
|---|---|---|
| 구 기준선 (T1 3회, 공통 26문항) | 11/26 | `20260917T070706`·`071057`·`072413`. 공통 문항 tool 변동 0건 |
| 사이클 2 (도구 재설계 9종 + 결정적 라우터) | 23/27 | 구 기준선 대비 +12 (분모 상이 — 구 기준선은 3회 공통 26문항) |
| 사이클 3 (분류 정의 정합) | 26/27 | 사이클 2 대비 +3 |
| 참고: 라우터 단독 (정답 카테고리 주입) | 27/27 | 오케스트레이터 전달 (tuba 측정). 분류기 오차를 제외한 라우터 상한이다 |

사이클 3 카테고리별 (직접 실행 확인):
...
### 사이클 3: 분류 정의 정합 (과적합이 아닌 근거)

사이클 2 드라이런의 틀린 4건(C1-07·C2-03·C2-05·C4-04)은 전부 캐시 분류가 정답 분류와 어긋난 문항이었다. 어긋난 축 4개는 구 프롬프트 정의와 매핑표 정의의 차이와 대응한다. 구 정의(`88ef60e^:src/prompts.ts`에서 확인)는 HISTORY에 찜 개수, TASTE에 기피·갭, DATA_OPS에 결측 현황이 없었고, OUT_OF_SCOPE는 "미보유 게임 평가"처럼 넓었다.
...
- SUBJECTIVE를 "게임 1건의 주관 기록 … 과 주관 필드 기준 게임 목록"으로 범위를 박고, "장르 집계·분포·갭 교차는 TASTE다"라는 carve-out 추가.
- DATA_OPS에 "필드 결측 현황(어느 정보가 비었나·있나)" 추가.
- OUT_OF_SCOPE를 "새로 수집해야 답할 수 있는 것만"으로 좁히고, "기록 확인은 목록에 없어도 범위 안" 추가.

과적합이 아닌 이유: 매핑표는 명세가 요구한 필수 산출물이며 정답지보다 먼저 만들어진 설계 문서다. 프롬프트를 표에 맞춘 것은 정답 맞추기가 아니라 일관성 회복이다. 규칙 조건에 문항 id·특정 게임명·질문 문자열을 넣지 않았다는 과적합 금지선도 그대로 지켰다(계약 v3에서 확인).

### 틀린 1건: C2-05

C2-05("짧게 플레이했는데 별점이 높은, 강렬했던 게임이 뭐야?")는 분류기가 SUBJECTIVE를 냈고, 기대 `find_rating_playtime_gaps`+`search_docs` 대신 `get_game_note`+`lookup_library`+`get_wishlist`+`search_docs` 4도구가 나갔다(직접 실행 확인). 거울 문항 C2-04는 TASTE로 맞았고, tuba의 체인 측정 3회에서는 C2-05도 TASTE였다(오케스트레이터 전달). 단일 잡음으로 판단해 추가 수정하지 않았다. TASTE 정의에 이미 갭 교차가 있어 더 깎을 매핑 근거가 없었다는 점도 판단 근거다.

### 드라이런 방법론

드라이런(`src/dryrun.ts`)은 classify 출력을 캐시(`data/classify_cache.json`, 30문항)에 1회 고정하고 라우터만 반복 채점한다. 기본 실행은 LLM을 한 번도 부르지 않는다. 채점은 `evaluate.ts`의 `toolScoreFor`를 import해 본측정과 같은 규칙(순서 무시·정확 일치·개수 일치)으로 잰다.

본측정을 대체하지 않고 선행 점검용이다. 답변 지표는 여기서 못 얻는다 — 드라이런 점수는 전부 tool 지표다.

정답지에 gameTitles를 적는 대신 캐시를 쓴 이유: 사람이 쓴 정답을 주입하면 분류기 성능이 은폐된다. 캐시는 분류기 출력을 그대로 얼리므로, 틀린 분류가 드라이런에 그대로 드러난다(사이클 2의 4건이 그 예다).

도구 선택 LLM을 걷어내 문항당 LLM 호출은 3회에서 2회로 줄었다(classify·answer. `88ef60e` 커밋 메시지에 기록). 측정 시간과 비용에 직접 영향이 있다.
...
화면(`public/index.html`, 의존성 없는 단일 HTML)은 챗봇 레이아웃이다. 질문·답변 말풍선이 위에 누적되고, 아래 입력창에서 계속 물을 수 있다. 과제 필수 요구(답변과 함께 호출한 도구·근거 스니펫·검증 결과 노출)는 말풍선 아래 붙는 부속 요소로 만족한다.
...
- **평가셋을 측정 사이에 고쳐 비교를 오염시켰다.** C5-02의 기대 도구를 1차 측정 뒤 `escalate`에서 `search_docs|escalate`로 수정했다. 두 결과 JSON에 서로 다른 `expectedTools`가 남아 있어 사후 확인은 된다(현행 `data/eval_set.csv`에도 수정된 값이 들어 있다). 그러나 그때는 이미 비교가 망가진 뒤다. 2차의 tool -1이 성능 저하인지 기준 변경인지 구분되지 않는다((4)절 검산표의 "오염" 행). 배운 것: 측정 도구는 측정 중에 건드리지 않는다. 기준을 고쳐야 한다면 이전 회차를 새 기준으로 다시 돌리거나, 고친 시점을 경계로 비교를 끊어야 한다.
- **측정이 비결정적이라는 것을 사이클 두 번을 돌고서야 알았다.** 심판 LLM이 같은 답변에 다른 판정을 냈다. C5-03은 `escalate` 고정 문구인데 1차 0점 → 2차 1점이다. 단 1차 답변 원문은 1차 JSON에 기록되지 않아 문자열 동일의 직접 대조는 불가하고, `escalate.ts`의 고정 문구와 2차 답변 원문으로 뒷받침된다. 분류도 뒤집혔다. C1-07 동일 질문이 1차에는 `get_taste_profile` 경로였는데 2차에는 `OUT_OF_SCOPE`(confidence 0.4, 2차 JSON의 `predictedCategory`·`confidence`·`escalated`로 확인)로 분류돼 이관됐다. 이관 임계값 0.6 근처 문항은 회차마다 결과가 달라진다. 배운 것: 1회 측정의 ±1~2문항 차이를 개선 효과로 읽으면 안 된다. 변동 폭을 먼저 재고, 그보다 큰 변화만 효과로 해석해야 한다. 이 프로젝트는 그 순서를 반대로 밟았다.
- **규칙은 작동했는데 질문 문면이 걸리지 않았다.** `search_docs` 미호출을 "프롬프트 문구로는 못 고치는 층위의 문제"로 진단했다. 그러나 회복된 문항과 실패한 문항의 질문 문면(`data/eval_set.csv` 원문)을 대조하니 원인이 달랐다. 회복한 C4-06("Muck 장르가 비어 있는데 원래 장르 정보는 어디서 가져오는 거야?")은 규칙의 "출처" trigger에 걸린다. 실패한 C1-02("내 업적 달성률 제일 높은 게임이 뭐야?"), C2-01("내가 제일 좋아하는 장르가 뭐야?"), C3-02("Euro Truck Simulator 2 별점 몇 점 줬어?")는 전부 값을 묻는 문면이라 trigger에 걸리지 않는다. 규칙은 정확히 동작했고 적용 범위가 좁았을 뿐이다. 실패를 보고 층위 문제로 성급히 일반화한 것이 잘못이었다. 배운 것: 개선이 일부만 먹었을 때 "이 방법은 안 된다"로 넘어가기 전에, 먹은 사례와 안 먹은 사례의 차이를 먼저 봐야 한다. 차이가 방법의 층위가 아니라 적용 범위에 있을 수 있다.
- **독립 검토 둘이 갈린 지점에서 진실이 나왔다.** 같은 측정 데이터를 서로 모르게 두 곳에서 분석했다. 한쪽은 "프롬프트 층위로는 못 고친다"(구조 개편 필요), 다른 쪽은 "프롬프트 층위가 가장 크게 먹는다"(trigger 확대로 8문항 회복 가능)로 정반대 결론을 냈다. 앞 항목의 문면 대조로 검산한 결과, trigger 범위 설명이 관측과 맞았다. "8문항" 수치는 해당 검토의 주장으로, 이 문서의 JSON에서는 검산하지 않았다. 배운 것: 검토가 하나였으면 그 결론을 그대로 받아 구조 개편에 들어갔을 것이다. 결론이 갈리는 지점이 곧 근거가 약한 지점이고, 그곳을 데이터로 다시 확인하면 된다.
