...
function genreOf(question: string, deps: CollieDeps): string | null {
...
function selectHistory(question: string, deps: CollieDeps, primaryTitle: string | null): RoutedCall[] {
...
function selectTaste(question: string, deps: CollieDeps): RoutedCall[] {
...
function selectSubjective(question: string, deps: CollieDeps, primaryTitle: string | null): RoutedCall[] {
...
function selectDataOps(question: string, deps: CollieDeps): RoutedCall[] {
  void deps;
 ...
 * 1차 호출을 실제로 실행해 결측을 감지하므로 Promise다.
 */
export async function routeQuestion(
  ...
  for (const c of calls) {
    chunksByTool.set(c.tool, await executeTool(c.tool, c.args, deps, category));
  }
  ...
  if (lookupEmpty && !calls.some((c) => c.tool === 'get_wishlist')) {
    ...
    chunksByTool.set('get_wishlist', await executeTool('get_wishlist', {}, deps, category));
  }
  ...
  for (const c of calls) {
    ...
    const entry = await findNoteEntry(arg);
