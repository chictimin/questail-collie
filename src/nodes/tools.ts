...
export async function findNoteEntry(titleOrAppid: string): Promise<NoteEntry | null> {
  ...
  const byId = entries.find((e) => e.gameId !== '' && e.gameId === q);
  if (byId) return byId;
  const exact = entries.find((e) => e.title !== '' && norm(e.title) === norm(q));
...
export async function runGameNote(
  ...
  const hit = await findNoteEntry(q);
...
export async function runWishlist(profile: TasteProfile, category: QueryCategory): Promise<EvidenceChunk[]> {
  const entries = await loadNoteEntries();
  return runWishlistSync(profile, (gameId) => {
    const hit = entries.find((e) => e.gameId === gameId);
...
export async function runRatingPlaytimeGaps(
  rawArgs: Record<string, unknown>,
  deps: CollieDeps,
  category: QueryCategory,
): Promise<EvidenceChunk[]> {
...
// isEmptyResult 대상이 아니다). rating만 주관(노트) 조회라 async다.
...
export async function runFieldCoverage(
  rawArgs: Record<string, unknown>,
  library: LibraryIndex,
  category: QueryCategory,
): Promise<EvidenceChunk[]> {
...
export async function executeTool(
  ...
  switch (tool) {
    ...
    case 'get_wishlist':
      return runWishlist(deps.profile, category);
    case 'find_rating_playtime_gaps':
      return runRatingPlaytimeGaps(args, deps, category);
    case 'get_field_coverage':
      return runFieldCoverage(args, deps.library, category);
